...
function createContextMenu() {
  chrome.contextMenus.create({
    id: 'browser-publish-open-editor',
    title: '小叶发布器 - 提取并编辑文章',
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  if (info.menuItemId === 'browser-publish-open-editor' && tab?.id) {
    try {
      // 获取 DSL 平台
      const dslPlatforms = await checkAllPlatformsAuth(false)
...
chrome.runtime.onInstalled.addListener(async details => {
  ...
  if (details.reason === 'update') {
    ...
    if (
      ...
      logger.info('Update detected, external changelog disabled for personal build.')
    }
  }

  // 首次安装时打开欢迎页
  if (details.reason === 'install') {
    logger.info('Install welcome page disabled for personal build.')
  }
})

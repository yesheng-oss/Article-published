...
const ARTICLE_EXTRACT_TIMEOUT_MS = 1200
const ARTICLE_EXTRACT_RETRIES = 3
...
function isValidArticle(article: unknown): article is Article {
  if (!article || typeof article !== 'object') return false
  const candidate = article as Partial<Article>
  return Boolean(candidate.title?.trim() && candidate.content?.trim())
}

async function requestArticleFromContentScript(tabId: number): Promise<Article | null> {
  for (let attempt = 0; attempt < ARTICLE_EXTRACT_RETRIES; attempt++) {
    try {
      const response = await Promise.race([
        chrome.tabs.sendMessage(tabId, { type: 'EXTRACT_ARTICLE' }),
        wait(ARTICLE_EXTRACT_TIMEOUT_MS).then(() => null),
      ]) as { article?: Article } | null

      if (isValidArticle(response?.article)) {
        return response.article
      }
    } catch (error) {
      logger.debug('Content script article extraction attempt failed:', error)
    }
...
async function extractArticleWithPageFallback(tabId: number): Promise<Article | null> {
  try {
    const results = await chrome.scripting.executeScript({
      ...
      func: () => {
        ...
        return {
          ...
          summary,
          cover,
          source: { url: window.location.href, platform: window.location.hostname.includes('mp.weixin.qq.com') ? 'weixin' : 'web' },
        }
      },
    })
    ...
    logger.error('Fallback article extraction failed:', error)
    return null
  }
}
...
export const useSyncStore = create<SyncState>((set, get) => ({
  ...
  loadArticle: async () => {
    ...
    try {
      ...
      const article = await requestArticleFromContentScript(tab.id)
        || await extractArticleWithPageFallback(tab.id)
      logger.debug('loadArticle - extracted article:', article?.title)
      if (article) {
        set({ article })
        // 追踪内容特征

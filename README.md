...
- 你经常把同一篇文章发布到多个平台。
- 你希望 AI 帮你整理文章，但不想把平台账号密码交给 AI。
- 你希望通过 MCP 让 Claude、Codex 等工具控制浏览器侧发布流程。
- 你更倾向本地运行，不依赖第三方云服务。
...
```text
packages/
  core/        平台适配器、内容处理和同步核心逻辑
  extension/   Chrome/Edge Manifest V3 浏览器扩展
  mcp-server/  本地 MCP 服务和浏览器 WebSocket 桥接
```
...
### 为什么不是直接安装 Chrome 商店版本？

这个项目目前更适合通过 GitHub 源码或 Release 包本地安装。用户需要开启浏览器开发者模式，然后加载 `packages/extension/dist`。

### 为什么要保持浏览器登录？

小叶发布器使用浏览器已有登录态保存草稿，不会要求你把平台账号密码交给 AI 客户端。

### 为什么只保存草稿，不直接发布？

为了降低误发风险。AI 可以辅助整理和创建草稿，但最终发布建议由你人工确认。

### 扩展提示没有检测到文章怎么办？

先刷新文章页，再点击扩展图标。如果仍然失败，确认当前页面是正文页面，而不是平台首页、搜索页或登录页。

### 图片上传失败怎么办？

部分平台会限制外链图片或防盗链。遇到图片上传失败时，可以先保存草稿，再在平台后台手动补图。

## GitHub Releases 分发建议

如果你维护这个仓库，可以在每次构建后发布一个 Release：

1. 运行 `npm run build`。
2. 打包 `packages/extension/dist` 里的内容为 zip。
3. 在 GitHub Releases 上传 zip。
4. 在 Release 说明里写清楚：下载、解压、开发者模式加载 `dist`。
